...
same scene, same episodes, different training data. Behaviourally Clean's aim
does not move with the cube (spread 1.0 deg across cells) and Randomized's does
(10.2 deg). The question is whether the representations agree.
...
def main():
    ...
    for path in sorted(glob.glob(os.path.join(INDIR, "activations_*.npz"))):
        ...
        print(f"{slug}: {len(set(ep))} episodes, {len(y)} samples, "
              f"by-episode {np.median(err_ep):.1f} deg, "
              f"by-position {np.median(err_pos):.1f} deg, chance {np.median(chance):.1f}")
